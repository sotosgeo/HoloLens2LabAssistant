...
public class ArucoTracker : MonoBehaviour
{
    ...
    public Action<IReadOnlyDictionary<int, Marker>> onDetectionFinished;
    ...
    private IReadOnlyDictionary<int, Marker> DetectMarkers(SoftwareBitmap softwareBitmap)
    {
        ...
        foreach (var marker in markers)
        {
            ...
            var markerInUnity = new Marker(marker.Id, posInUnity, rotInUnity);
            markersInUnity.Add(marker.Id, markerInUnity);
        }

        return markersInUnity;
    }

    private IReadOnlyDictionary<int, Marker> DetectBoard(SoftwareBitmap softwareBitmap)
    {
        // Get marker detections from opencv component
        ...
        if (isDetected)
        {
            ...
            var markerInUnity = new Marker(-1, unityPosition, unityRotation);
            
            markersInUnity.Add(markerInUnity.id, markerInUnity);
        }

        return markersInUnity;
    }

    //https://github.com/microsoft/MixedReality-SpectatorView/blob/7796da6acb0ae41bed1b9e0e9d1c5c683b4b8374/src/SpectatorView.Unity/Assets/PhotoCapture/Scripts/
    //.cs#L1256
    /// <summary>
